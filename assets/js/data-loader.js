/* ============================================================
   OCHRE MOROCCO — Dynamic Data Loader v1.0
   Loads content from data/*.json and renders it live.
   Settings (contact, social) also applied site-wide.
   ============================================================ */
...
(function () {
  ...
  function cardHTML(exc) {
    ...
    return '<div class="card" data-exc-id="' + exc.id + '">' +
      '<div class="card-img">' +
        ...
        discount + tag +
        '<div class="card-price-overlay"><span class="price-new">&euro;' + exc.price + '</span>' + oldPrice + '</div>' +
      '</div>' +
      '<div class="card-body">' +
  ...
  async function renderExcursionsPage() {
    ...
    try {
      ...
      /* Trigger booking button init if booking.js already loaded */
    ...
    try {
      ...
      if (s.social) {
        ...
        Object.keys(map).forEach(function(k) {
          if (s.social[k]) {
            document.querySelectorAll('a[href*="' + map[k] + '"]').forEach(function(a) { a.href = s.social[k]; });
          }
        });
      }
    } catch (e) { /* silent — settings not critical */ }
  }
  ...
  window.OchreDataLoader = { renderExcursionsPage: renderExcursionsPage, applySettings: applySettings };
})();
